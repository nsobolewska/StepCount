...
class UI(FloatLayout):#the app ui
	...
	def __init__(self, **kwargs):
		...
		self.btn5 = Button(text="Przeslij", width=50,height=50)
		self.btn5.bind(on_press=self.socke)
	...
	def socke(self,instance):
		...
		pocz = "A"
		self.s.send(str.encode(pocz))
		for line in akc:
			self.s.send(str.encode(line))
			time.sleep(1/200)
		pocz = "K"
		self.s.send(str.encode(pocz))
	...
	def update(self, dt):
		...
		try:
			self.tekst = self.tekst + "	%.2f	%.2f	%.2f	"%(		#x,y,z from accelerometer
			accelerometer.acceleration[0], #read the X value
			accelerometer.acceleration[1], # Y
			accelerometer.acceleration[2]) # Z
		except:
